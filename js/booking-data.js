/* ============================================================================
   ...
   удобного времени, а не бронирование: заявка уходит администратору
   в Telegram, и он перезванивает подтвердить точное время.
...
window.DOUBLECUTS = {
  ...
  /* Куда падают заявки с сайта — сообщением в Telegram-бота.
     1) Создайте бота через @BotFather в Telegram — получите botToken.
     2) Напишите этому боту любое сообщение (например «привет») —
        без этого он не сможет писать вам первым.
     3) Узнайте chatId: откройте в браузере
        https://api.telegram.org/bot<ВАШ_ТОКЕН>/getUpdates
        после шага 2 там будет "chat":{"id": ...} — это и есть chatId.
     Впишите оба значения ниже. Пока поля пустые — заявки только
     сохраняются в localStorage браузера гостя и никуда не отправляются
     (см. README.md, раздел «Онлайн-запись»). */
  telegram: {
    botToken: '',
    chatId: ''
  },
